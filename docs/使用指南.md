...
如果使用 `latest` 标签，更新非常简单：

```bash
# 删除 lock 文件，重新安装（会自动拉取 latest 标签指向的最新版本）
...
**特点**：
- ✅ 自动获取最新稳定版本（latest 标签指向的版本）
- ✅ 无需手动更新版本号
- ✅ 适合生产环境（使用稳定版本，不是开发分支）
...
**更新方式**：
```bash
# 删除 lock 文件，重新安装（会自动拉取 main 分支最新代码）
rm -rf node_modules pnpm-lock.yaml
pnpm install
```

**注意**：main 分支可能包含未测试的代码，不推荐生产环境使用。
